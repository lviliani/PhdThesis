...
The analysis presented here is based on the published inclusive \hwwllnn measurements by CMS~\cite{Chatrchyan:2013iaa}, modified to be inclusive in the number of jets. 
...
For jets with $\pt>30$\GeV, a better performing algorithm, JP, is used\footnote{The default algorithm used in the 8\TeV inclusive analysis was TCHE. Here the event selection has been optimized to perform an inclusive analysis in number of jets, and the usage of a better performing b tagging algorithm (JP) has been preferred for events containing high \pt jets.}. Jets are identified as b-jets by the JP algorithm if the discriminating variable has a value above 1.4.
